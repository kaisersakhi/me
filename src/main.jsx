...
ReactDOM.createRoot(document.getElementById('root')).render(
  <React.StrictMode>
    <section className='h-screen' style={{position: 'relative'}}>
      <img src={img[(Math.floor(Math.random() * img.length))]} alt="Image"  style={{ filter: 'blur(8px)' }} className='w-full h-full object-cover'/>
      <div className='flex items-center w-full h-full column' style={{ position: 'absolute', top: 0 }}>
        <h1 className='text-white p-10 text-center text-6xl w-full' style={{fontFamily: "BrittnaySignature"}}>Kaiser Sakhi</h1>
      </div>
